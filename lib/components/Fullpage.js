...
class ReactToWindowChange extends React.Component {
  componentDidUpdate(oldProps) {
    if (oldProps.height !== this.props.height) {
      const to = this.props.activeSlide * this.props.height;
      this.props.verticalRoot.scrollTop = to;
  }
...
class Fullpage extends React.Component {
  constructor(props) {
    ...
    const scrollByPage = Math.floor(window.scrollY / window.innerHeight);

    const {activeSlide = scrollByPage, scrollSensitivity, touchSensitivity, scrollSpeed = 500} = p;
    ...
    this.onSlideChangeStart(this.name, this.props, this.state, this.state);
    this.onSlideChangeEnd(this.name, this.props, this.state, this.state);
  }
      ...
      if (!this.props.frozen) {
        ...
        const tryScroll = () => {
          if (!document.body.offsetHeight) {
            ...
            setTimeout(() => {
              const scrollByPage = Math.floor((window.scrollY || window.pageYOffset) / window.innerHeight);
              if (scrollByPage !== this.state.activeSlide) {
                this.setState({activeSlide: scrollByPage}, () => {
                  this.onSlideChangeStart(this.name, this.props, this.state, this.state);
                  this.onSlideChangeEnd(this.name, this.props, this.state, this.state);
                });
              }
          ...
          return (
            <div ref={node => (this.node = node)} className="Fullpage">
              <ReactToWindowChange
                ...
                verticalRoot={this.verticalRoot}
              ...
              {slides.map((sl, i) => {
                ...
                if (sp.slides) {
                  const {name} = sp;
                  return <HorizontalSlider cache={this.cacheHslide.bind(this)} width={width} height={height}
                                           window={window} document={document} activeSlide={s[name].activeSlide}
                                           hideScrollBars={p.hideScrollBars} {...sp} key={i}>
                    {children}
                  </HorizontalSlider>;
                }

                return <Slide render={true} className={sp.className || ''} id={sp.id} width={width} height={height}
                              key={i} {...sp}>
                  {children}
                </Slide>;
              })}
